...
func init() {
	Fun.Register(&Cmd{
		...
		Handler: func(m twitch.PrivateMessage) (err error) {
			...
			rows, err := logs_db.Clickhouse.Query(context.Background(), "SELECT channel_id, channel_login, count() AS lines FROM rustlog_zonian.message_structured WHERE user_id=? GROUP BY channel_id ORDER BY lines DESC", user.ID)
			if err != nil {
				return
			}
			defer rows.Close()
